...
export class Soundscape {
  ...
  constructor(ctx, out, opts = {}) {
    ...
    this.wHP = bq('highpass', 55, 0.7);
    this.nA.connect(this.wHP);

    this.wLow = bq('lowpass', 220, 0.7);
    this.gLow = g(1.0);
    this.wHP.connect(this.wLow).connect(this.gLow).connect(sp(-0.18)).connect(this.windBus);

    this.wBody = bq('bandpass', 480, 0.75);
    this.gBody = g(0.55);
    this.nB.connect(this.wBody).connect(this.gBody).connect(sp(0.22)).connect(this.windBus);

    this.wWhis1 = bq('bandpass', 1250, 9);
    this.gWhis1 = g(0);
    this.nB.connect(this.wWhis1).connect(this.gWhis1).connect(sp(-0.52)).connect(this.windBus);

    this.wWhis2 = bq('bandpass', 2100, 15);
    this.gWhis2 = g(0);
    this.nA.connect(this.wWhis2).connect(this.gWhis2).connect(sp(0.58)).connect(this.windBus);
  ...
  windAt(t, out) {
    ...
    const base = clamp(
      0.155 + 0.085 * Math.sin(t * 0.0431 + 1.3) + 0.05 * Math.sin(t * 0.0177 + 0.4) +
      0.03 * Math.sin(t * 0.0091 + 2.7), 0.04, 0.42);
    let gsum = 0, turn = 0;
  ...
  _scheduleWind(now) {
    ...
    while (this.schedHead < now + HORIZON) {
      ...
      ramp(this.windBus.gain, 0.030 + 0.30 * gg * gg + 0.055 * gg, t);
      ...
      ramp(this.gWhis1.gain, 0.30 * edge * edge * wallGate, t);
      ramp(this.gWhis2.gain, 0.19 * edge * edge * edge * wallGate, t);
  ...
  _scheduleSand(gust) {
    if (gust.sand) return;
    gust.sand = true;
    if (gust.peak < 0.28) return;                 // a light gust moves nothing
    const r = this.erand;
    ...
    for (let i = 0; i < n; i++) {
      ...
      burst(this.sandGain.gain, at, 0.055 * strength, 0.09 + r() * 0.2, dur);
      this.sandBP.frequency.setValueAtTime(1700 + r() * 2800, at);
      // Grains trickling down the bank, following the burst rather than
      // sitting on top of it.
      if (r() < 0.75) {
        const gt = at + 0.15 + r() * 0.5;
        burst(this.trickleGain.gain, gt, 0.030 * strength, 0.05, 0.5 + r() * 1.9);
  ...
  _step(t, x, z, u, speed) {
    ...
    const bodyLvl = lerp(0.055, 0.026, gravel) * vary * load;
    ...
    const crLvl = lerp(0.022, 0.05, gravel) * vary * load;
