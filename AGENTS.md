...
- **The bake store (`src/bake.js`)**: the generated textures, the heavy geometry and the clast scatter are cached in IndexedDB (15 entries, ~98 MB), so only the first load on a given browser pays full price — 69 s cold, 10 s warm.
