...
- **The bake store (`src/bake.js`)**: the heavy generated arrays are cached in IndexedDB, so only the first load on a given browser pays full price. Three things about it are worth knowing before you debug anything odd:
