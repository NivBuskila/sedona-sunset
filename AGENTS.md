...
- **The bake store (`src/bake.js`)**: the generated textures, the heavy geometry and the clast scatter are cached in IndexedDB (21 entries, ~106 MB), so only the first load on a given browser pays full price — 72 s cold, 9.5 s warm.
